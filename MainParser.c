...
/* External objects */
extern PZY_int syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern PZY_int line; /* source code line number - defined in scanner.c */
...
extern PZY_int startScanner(BufferPointer sc_buf);

static PZY_void  printParserError(PZY_char* fmt, ...);
static PZY_void  displayParser(BufferPointer ptrBuffer);
static PZY_long getParserFileSize(PZY_char* fname);
static PZY_void  callGarbageCollector(void);
...
PZY_int mainParser(PZY_int argc, PZY_char** argv) {
...
PZY_void printParserError(PZY_char* fmt, ...) {
...
PZY_long getParserFileSize(PZY_char* fname) {
	FILE* input;
	PZY_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
void callGarbageCollector(PZY_void){
